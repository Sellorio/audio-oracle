...
public abstract class MapperBase
{
    ...
    protected MapperBase(params object[] additionalMappers)
    {
        ...
        {
            MapMethods[mapImplementation] = (this, mapImplementation.GetMethod(nameof(IMap<object, object>.Map)))!;
        }
    }
    ...
    private object? Map(object? from, Type toType, bool calledFromMapMethod)
    {
        ...
        {
            var hasValue = (bool)fromType.GetProperty(nameof(Nullable<int>.HasValue))!.GetValue(from)!;
            ...
            var fromValue = fromType.GetProperty(nameof(Nullable<int>.Value))!.GetValue(from)!;
    ...
    private object AutoMap(object from, Type toType)
    {
        ...
        {
            if (toType != typeof(string) && typeof(IEnumerable).IsAssignableFrom(toType))
            {
                return AutoMapEnumerable(from, toType);
            }
