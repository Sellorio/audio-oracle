...
using Sellorio.AudioOracle.Library.Validation.Validators;
using System;
...
internal class ValidationBuilder<TObject>(TObject target, IServiceProvider serviceProvider) : IValidationBuilder<TObject>
{
    ...
    {
        Messages.Add(new ResultMessage(message, severity, null));
    ...
    public IValidationBuilder<TObject> AddMessage<TPathValue>(Expression<Func<TObject, TPathValue>> path, string message, ResultMessageSeverity severity = ResultMessageSeverity.Error)
    {
        ...
        Messages.Add(new ResultMessage(message, severity, messagePath));
            ...
            finally
            {
                Messages.AddRange(
                    childValidationBuilder.Messages
                        .Select(x =>
                            new ResultMessage(x.Text, x.Severity, x.Path == null ? null : Enumerable.Concat(messagePath, x.Path).ToImmutableArray())));
            }
        }
