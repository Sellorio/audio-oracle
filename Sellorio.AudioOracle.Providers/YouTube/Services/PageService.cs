...
internal partial class PageService(HttpClient httpClient) : IPageService
{
    public async Task<IList<JsonNavigator>> GetPageInitialDataAsync(string url)
    {
        ...
        await RateLimiters.YouTubePageFetch.WithRateLimit(async () =>
            responseMessage = await httpClient.GetAsync(url));

        responseMessage.EnsureSuccessStatusCode();
        var responseText = await responseMessage.Content.ReadAsStringAsync();

        var result = new List<JsonNavigator>(3);
        var index = 0;
    ...
    {
        return XmlEscapeSequencesRegex().Replace(xmlEncodedText, x =>
    ...
    [GeneratedRegex(@"(\\x22|\\x27|\\x7b|\\x7d|\\x5b|\\x5d|\\x3d|\\\\|\\)")]
    private static partial Regex XmlEscapeSequencesRegex();
}
